...
\begin{abstract}
We describe \texttt{eclingo}, a solver for epistemic specifications under Gelfond 1991 semantics built upon the \texttt{clingo} Answer Set Programming system. The input language of \eclingo{} uses the syntax extension capabilities of \clingo{} to define subjective literals that, as usual in epistemic specifications, allow checking the truth of a regular literal in all or in some of the answer sets of a program.
The \eclingo{} solving process follows a guess and check strategy so it first generates potential truth values for subjective literals and, in a second step, it checks the obtained result with respect to the cautious and brave consequences of the program.
This process is implemented using the multi-shot feature of \clingo.
We have also implemented some optimisations, aiming to reduce the search space and, therefore, increasing \eclingo{}'s efficiency in some scenarios. 
Finally, we compare the efficiency of \texttt{eclingo} with two state-of-the-art solvers for epistemic specifications on a pair of benchmark scenarios and show that it generally outperforms their obtained results.
\end{abstract}
\begin{keywords}
