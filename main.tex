...
%
Notice that, once a subjective atom {\tt \&k\{$lit$\}} is replaced by an standard auxiliary atom $\mathit{aux}$, the relation to the original literal is lost.
%
...
%
This situation improves the efficiency of \eclingo{} whenever a world view is only proposed by inconsistent answer sets.
We can also agree that these constraints can help in the guess phase since the set of answer sets produced by this invocation to \clingo\ tends to be smaller.

%\subsubsection{Completion}
Another optimization that \eclingo{} performs consists in extending the auxiliary logic program with a new set of auxiliary rules,
...
\end{eqnarray}
that may allow for concluding more information once the subjective atoms are replaced by auxiliary ones.
These rules are built taking advantage of the parsing process, where the rules are translated to its auxiliary representation. In this way, every time
...
This auxiliary completion rule is a copy of the auxiliary one, where objective literals are replaced under the following policy:
\begin{align*}
...
Another optimisation is based on the iterative computation of the well-founded model of the auxiliary guess program.
...
%\end{theorem}
If, instead of computing the answer sets of the program, we just use \gringo\ to obtain its well-founded model (WFM),
we get a good estimate of regular atoms that are always true (resp.\ always false) in all the answer sets of the program.
...
In this section,
we compare \eclingo{} with two other epistemic solvers, \wviews~\cite{Kelly07} and \EPASP~\cite{SLKL17}, both with respect to usability and efficiency.
...
%
Since the three tools have different dependencies, their installation in the same machine was infeasible. For this reason, a set of \texttt{Amazon Web Services}~\footnote{\url{https://aws.amazon.com/}} virtual machines have been used. These virtual machines are instances of type \texttt{t2.micro}, whose technical specifications are:
...
\begin{table}[ht]
    ...
    \cline{1-5}
    & \texttt{\wviews{} G91} & \texttt{eclingo G91} & \texttt{\EPASP{} K15} & \texttt{eclingo K15} \\
    \cline{1-5}
    ...
    \texttt{eligible25} & -          & {\bf 0.065}       & -          & {\bf 0.789}       \\
    \cline{1-5}
    \end{tabular}
...
\begin{table}[ht]
    ...
    \cline{1-4}
           & \texttt{\EPASP{} K15, planning mode} & \texttt{eclingo G91} & Path length \\
    \cline{1-4}
...
\begin{table}[ht]
    \begin{tabular}{c|cc|c}
    \cline{1-4}
                       & \texttt{\wviews{} G91} & \texttt{\eclingo{} G91} & Path length \\
    \cline{1-4}
        ...
        \texttt{ground\_yale13*} & -         & {\bf 3.067}      & 10      \\
    \cline{1-4}
        \end{tabular}
